...
/** Creates a brand-new restaurant (a signup), seeds it with a starter menu,
 * and returns the created record — or null if the slug is already taken. */
export async function signUpRestaurant(slug: string, name: string): Promise<RestaurantRecord | null> {
  const created = await createRestaurantRemote(slug, name);
