...
function ResolvedRestaurant({ slug }: { slug: string }) {
  ...
  return (
    <RestaurantProvider restaurantId={restaurant.id}>
      <RestaurantRouter slug={slug} />
    </RestaurantProvider>
  );
}

function RestaurantRouter({ slug }: { slug: string }) {
  const { settings } = useSettings();
  const [authed, setAuthed] = useState(() => sessionStorage.getItem(`rbs_admin_auth_${slug}`) === '1');
  ...
  if (route === 'admin') {
    if (!authed) {
      return (
        <AdminLogin
          settings={settings}
          onSuccess={() => {
            sessionStorage.setItem(`rbs_admin_auth_${slug}`, '1');
            setAuthed(true);
          }}
        />
      );
    ...
    return (
      <AdminDashboard
        onLogout={() => {
          sessionStorage.removeItem(`rbs_admin_auth_${slug}`);
          setAuthed(false);
        }}
      />
    );
...
function PlatformLanding() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const created = await signUpRestaurant(effectiveSlug, name.trim());
      if (!created) {
        setError('That URL is already taken — try a different one.');
        setSubmitting(false);
        return;
      }
      window.location.href = `/${created.slug}#admin`;
    } catch (err: any) {
